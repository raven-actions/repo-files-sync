...
/**
 * Parse exclusion patterns from a newline-separated string
 */
function parseExclude(text: string | undefined, src: string): string[] | undefined {
    ...
    .map((item): FileConfig | undefined => {
      ...
      if (fileItem.source !== undefined) {
        return {
          ...
          deleteOrphaned: fileItem.deleteOrphaned === undefined ? DELETE_ORPHANED_DEFAULT : fileItem.deleteOrphaned,
          exclude: parseExclude(fileItem.exclude, fileItem.source)
        };
      }
